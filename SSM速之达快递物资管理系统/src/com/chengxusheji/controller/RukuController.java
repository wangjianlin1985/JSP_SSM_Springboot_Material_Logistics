...
public class RukuController extends BaseController {
	...
	public void add(Ruku ruku, BindingResult br,
		...
		Wuzi wuzi = wuziService.getWuzi(ruku.getWuziObj().getWuliuNo());
		
		ruku.setWuziClassObj(wuzi.getWuziClassObj()); 
		ruku.setZhongliang(wuzi.getZhongliang()); 
		WorkMan workManObj = new WorkMan();
		workManObj.setWorkUserName(session.getAttribute("workman").toString());
		ruku.setWorkManObj(workManObj); 
		
		Cangku cangku = cangkuService.getCangku(ruku.getCangkuObj().getCangkuId());
        ...
        wldt += "\r\n" + sdf.format(new java.util.Date()) + "入库： " + cangku.getCangkuName() + " " + ruku.getRukuMemo();
        wuzi.setWldt(wldt);
