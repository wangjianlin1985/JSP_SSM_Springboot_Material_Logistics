...
public class ChukuController extends BaseController {
	...
	/*客户端ajax方式提交添加出库信息*/
	@RequestMapping(value = "/workmanAdd", method = RequestMethod.POST)
	public void workmanAdd(Chuku chuku, BindingResult br,
		...
		Wuzi wuzi = wuziService.getWuzi(chuku.getWuziObj().getWuliuNo());
		
		chuku.setWuziClassObj(wuzi.getWuziClassObj()); 
		chuku.setZhongliang(wuzi.getZhongliang()); 
		WorkMan workManObj = new WorkMan();
		workManObj.setWorkUserName(session.getAttribute("workman").toString());
		chuku.setWorkManObj(workManObj); 
		
		Cangku cangku = cangkuService.getCangku(chuku.getCangkuObj().getCangkuId());
		
		chukuService.addChuku(chuku);
        
        
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String wldt = wuzi.getWldt();//物流动态
        wldt += "\r\n" + sdf.format(new java.util.Date()) + "出库于： " + cangku.getCangkuName() + " " + chuku.getChukuMemo();
        wuzi.setWldt(wldt);
